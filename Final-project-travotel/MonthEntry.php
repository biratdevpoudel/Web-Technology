...
<?php 
 ...
 /* $proc = "Delimiter $$
  ...
  $q1 = "select * from reservation where approval = 'Not Approved'";
  $res = mysqli_query($db,$q1);
  $db->next_result();
  
  echo "<table>";
  echo "<tr><th><b>Username</b></th>
  <th><b>Hotel</b></th>
  <th><b>Location</b></th>
  ...
  while($row = mysqli_fetch_array($res))
  {
    ...
    <td>{$in_date}</td>
    <td>{$out_date}</td>
    <form action='approval.php?id={$asd}' method='POST'> 
    <td><select name='approval' id = 'approval'>
    <option value='Approved'>Approve</option>
    <option value='Not_Approved'>Not Approve</option>
    <option value='' disabled selected>Choose option</option>
    </select></td>
    <td><input type='submit' value ='Submit' name='submit_app'></form></td></tr>";
  }
...
var message = url.split('?').pop()

if(message === 'Approved' || message === 'Not_Approved'){
  alert(message)
}
